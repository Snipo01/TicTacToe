#now that we test the TicTacToe and really works properly we are going to star!
...
module TicTacToe
  ...
  class ComputerPlayer < Player
    DEBUG = false #edit this if necessary

    def group_position_by_markers(line)
    ...
    def look_for_winning_or_blocking_position(opponent_marker)
      for line in LINES
        markers = group_position_by_markers(line)
        next if markers[nil].length != 1
        if markers [self.marker].length == 2
          log_debug "winnning on line #{line.join}"
    ...
    def random_prioritized_position
      log_debug "picking random position, favoring center and then cornrs"
...
#.new(ComputerPlayer, ComputerPlayer).play
#puts
players_with_human = [HumanPlayer, HumanPlayer].shuffle
